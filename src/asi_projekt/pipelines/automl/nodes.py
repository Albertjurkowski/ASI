...
def evaluate_automl(
    ...
    # Filtruj modele z NaN score (np. _FULL modele w good_quality)
    scored = leaderboard.dropna(subset=["score_val"])
    if scored.empty:
        scored = leaderboard

    best_model_name = scored.iloc[0]["model"]
    best_score_raw = scored.iloc[0]["score_val"]

    metrics_negated = [
        "root_mean_squared_error", "mean_absolute_error",
        "mean_squared_error", "median_absolute_error",
    ]
    best_metric_value = -best_score_raw if eval_metric in metrics_negated else best_score_raw

    logger.info(
        "AutoGluon: najlepszy model = %s, %s = %.4f",
        best_model_name, eval_metric, best_metric_value,
    )

    wandb.init(
        project=os.getenv("WANDB_PROJECT", "asi-projekt"),
        entity=os.getenv("WANDB_ENTITY"),
        name=f"automl-{parameters['automl']['presets']}-{parameters['automl']['time_limit']}s",
        config={
